...
describe('UserSQLDataSource.getUsers', () => {
  it('retorna lista de usuários sem filtros', async () => {
    const { db, queryBuilder } = makeDb();
    // sem filtros, queryBuilder é chamado como array
    db.mockImplementation(() => ({
      ...queryBuilder,
      // simula await query retornando array
      then: (resolve) => resolve([makeRow(), makeRow({ id: 2 })]),
    }));

    const ds = new UserSQLDataSource(db);
    ds.initialize({ context: {}, cache: undefined });

    // substitui diretamente o método para testar o reducer
    ds.db = () => ({
      ...queryBuilder,
      orderBy: jest.fn().mockReturnThis(),
      offset: jest.fn().mockReturnThis(),
      limit: jest.fn().mockReturnThis(),
      // knex query é thenable
      then: undefined,
    });
    ...
    const mockDb = jest.fn(() => {
      ...
      q[Symbol.iterator] = undefined;
      return Object.assign(Promise.resolve([makeRow(), makeRow({ id: 2 })]), q);
    });

    const ds2 = new UserSQLDataSource(mockDb);
  ...
  it('aceita colunas permitidas na whitelist', async () => {
    ...
    for (const col of allowed) {
      ...
      const ds = new UserSQLDataSource(mockDb);
      ds.initialize({ context: {}, cache: undefined });
